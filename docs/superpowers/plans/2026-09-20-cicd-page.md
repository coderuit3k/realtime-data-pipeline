...
- [ ] **Step 1: Ask the user to add the `GITHUB_TOKEN` Vercel env var**
...
then add it as a Vercel project environment variable named `GITHUB_TOKEN`
(same manual workflow as every AWS credential and the Upstash credentials
added in prior sub-projects). Wait for confirmation before proceeding.
